...
class No():
    ...
    def __init__(self,coor) -> None:
        ...
        elif not all( [ isinstance(c, self.__tipo_numero) for c in coor] ):
    ...
    def igdl(self, ig):
        ...
        elif not all( [(isinstance(i,self.__tipo_numero_inteiro) and i>=0) for i in ig]):
           raise ValueError(f'Os índices globais dos gdl do nó devem ser números naturais: {ig}')
    ...
    def definir_deslocamento(self,**kwargs):
        ...
        for dir, dp in kwargs.items():
            if dir not in self.__gdls_globais:
                raise ValueError('O deslocamento {} não pode ser prescrito para {}'.format(dir,self.__tipo))
            elif dir in aux:
                raise ValueError('O deslocamento {} tem mais de um valor prescritos'.format(dir))
            elif not isinstance(dp,self.__tipo_numero):
    ...
    def definir_apoio_elastico(self,**kwargs):
        ...
        for dir, k in kwargs.items():
            if dir not in self.__gdls_globais:
                raise ValueError('O apoio elastico em {} não pode ser definido para {}'.format(dir,self.__tipo))
            elif dir in aux:
                raise ValueError('O apoio elástico em {} tem mais de um valor de reigidez definido'.format(dir))
            elif not isinstance(k,self.__tipo_numero):
    ...
    def _check_cc(self):
        ...
        if not check:
            raise Exception('Há mais de um c.c. definida para os deslocamentos no nó')
    ...
    def definir_carga(self,**kwargs) -> None:
        ...
        if 'p' in kwargs:
            ...
            if p is None:
                self.__carga = ()
            elif len(p) == self.ngdl and all([isinstance(v,(int,float)) for v in p]):
            ...
            for i, p in enumerate(self.__forcas_globais):
                if p in kwargs:
                    ...
                    if isinstance(kwargs[p],(int,float)):
                        ...
                        raise ValueError(f'A carga {p} tem valor incorreto: {kwargs[p]}')
    ...
    def dados(self):
        ...
        if self.ngdlr>0:
            dados['gdl_restritos'] = [ self.__gdls_globais[i] for i in self.il_gdlr]
    ...
    def __repr__(self):
        return self.__class__.__name__ + f'( {self.coor},{self.igdl},{self.il_gdlr},{self.p})'
