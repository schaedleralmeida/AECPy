...
def rel_d_b3(xi, S, L, db3):
    ...
    M3 = (6 * EI) / (L**2) * (2 * xi - 1) * (u2i - u2j) + (2 * EI) / L * (
        (3 * xi - 2) * r3i + (3 * xi - 1) * r3j
    )
...
def rel_w2(xi, S, L, w2):
    ...
    Dw2 = w2[1] - w2[0]
    u_2 = (w2[0] * L**4) / (24 * EI) * (xi4 - 2 * xi3 + xi2) + (Dw2 * L**4) / (
        120 * EI
    ) * (xi5 - 3 * xi3 + 2 * xi2)
    rot_3 = (w2[0] * L**3) / (12 * EI) * (2 * xi3 - 3 * xi2 + xi) + (
        Dw2 * L**3
    ) / (120 * EI) * (5 * xi4 - 9 * xi2 + 4 * xi)
    V2 = -w2[0] * L / 2 * (2 * xi - 1) + Dw2 * L / 20 * (3 - 10 * xi2)
    M3 = w2[0] * L**2 / 12 * (6 * xi2 - 6 * xi + 1) + Dw2 * L**2 / 60 * (
        10 * xi3 - 9 * xi + 2
    )
...
def rep_T2(L, S, dT):
    ...
    return dT * S.cdtEI3 * np.array([0, -1, 0, 1])
...
def rel_d_b2(xi, S, L, db2):
    ...
    M2 = (6 * EI) / (L**2) * (2 * xi - 1) * (u3i - u3j) - (2 * EI) / L * (
        (3 * xi - 2) * r2i + (3 * xi - 1) * r2j
    )
...
def rel_w3(xi, S, L, w3):
    ...
    # carregamento com variação linear em todo o elemento
    Dw3 = w3[1] - w3[0]
    u_3 = (w3[0] * L**4) / (24 * EI) * (xi4 - 2 * xi3 + xi2) + (Dw3 * L**4) / (
        120 * EI
    ) * (xi5 - 3 * xi3 + 2 * xi2)
    rot_2 = -(
        (w3[0] * L**3) / (12 * EI) * (2 * xi3 - 3 * xi2 + xi)
        - (Dw3 * L**3) / (120 * EI) * (5 * xi4 - 9 * xi2 + 4 * xi)
    )
    V3 = -w3[0] * L / 2 * (2 * xi - 1) + Dw3 * L / 20 * (3 - 10 * xi2)
    M2 = w3[0] * L**2 / 12 * (6 * xi2 - 6 * xi + 1) + Dw3 * L**2 / 60 * (
        10 * xi3 - 9 * xi + 2
    )
...
def rep_T3(L, S, dT):
    ...
    return dT * S.cdtEI2 * np.array([0, 1, 0, -1])
