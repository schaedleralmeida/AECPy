...
def construir_rigidez_forças(nos:list[No], els:list[Elemento], ngdl:int) -> tuple(np.ndarray, np.ndarray):
    '''
...
def resolver(K:np.ndarray, F:np.ndarray, nos:list[No], ngdlF:int) -> tuple(np.ndarray, np.ndarray):
    '''
...
def resultados_nos(nos:list[No], U:np.ndarray, R:np.ndarray, index = None) -> pd.DataFrame:
    '''
