...
__format_diagrama_p = {'size':50,
                     'color_marker': 'g',
                     'marker': 'x'
                     }


__nomes_ordenada_modificado = {'u1':'$u_1$', 'u2':'$u_2$', 'u3':'$u_3$',
                               'r1':'$\\theta_1$', 'r2':'$\\theta_2$', 'r3':'$\\theta_3$',
                               'Mt':'$M_t$','M2':'$M_2$','M3':'$M_3$',
                               'V2':'$V_2$', 'V3':'$V_3$'}
...
def diagramas(relpts,resultados=None,conv=None):
    ...
    fig, axes_res = plt.subplots(figsize=[7,n_res*1.3],nrows=n_res, sharex=True)
    ...
    for rr, ax, in zip(res.items(), axes_res):
...
def criar_diagrama(ax,abscissa,ordenada,titulo_ordenada=['','']):
    ...
    if len(abscissa) != len(ordenada):
        raise ValueError('abscissas e ordenadas com número distindo de elementos')

    # insere para os valores positivos a cor color+ e para os valores negativos, a cor color-
    ax.fill_between(abscissa, ordenada, 0,
...
def modelo_3d(nos,els):
    ...
    estilo_elementos = {'linestyle':'-','color':'b','linewidth':5, 'alpha':1.0}
    ...
    folga = 1.2
    ax.set_box_aspect((folga*np.ptp(x), folga*np.ptp(y), folga*np.ptp(z))) 
