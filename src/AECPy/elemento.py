...
class Elemento():
    ...
    __parte_rigidez = {
        'ax': {'calc_K':axial.calc_Ka,      'rel_d':axial.rel_d_a,      'carga':'w1', 'rep':axial.rep_w1,   'rel_w':axial.rel_w1, 'dTemp': 'dTm', 'rep_dT': axial.rep_T, 'rel_dT':axial.rel_T},
        'tr': {'calc_K':axial.calc_Kt,      'rel_d':axial.rel_d_t,      'carga':'',
        'rep':None,  'rel_w':None, 'dTemp': '', 'rep_dT': None, 'rel_dT':None},
        'b2': {'calc_K':flexao.calc_Kb2,    'rel_d':flexao.rel_d_b2,    'carga':'w3', 'rep':flexao.rep_w3,  'rel_w':flexao.rel_w3, 'dTemp': 'dTv3', 'rep_dT': flexao.rep_T3, 'rel_dT':flexao.rel_T3},
        'b3': {'calc_K':flexao.calc_Kb3,    'rel_d':flexao.rel_d_b3,    'carga':'w2', 'rep':flexao.rep_w2,  'rel_w':flexao.rel_w2, 'dTemp': 'dTv2', 'rep_dT': flexao.rep_T2, 'rel_dT':flexao.rel_T2}
        }
    ...
    def iniciar(cls,tipo):
        ...
        if tipo =='TP': #elemento de treliça plana
            ...
            cls.__R_ord = None
            cls.__partes = {'ax':[0,6], 'tr':[3,9], 'b3':[1,5,7,11], 'b2':[2,4,8,10]} 
    ...
    def alterar_eixos_locais(self,alt):
        ...
        if self.tipo != 'PE':
            raise ValueError('A alteração dos eixos locais só é possível em Pórticos Espaciais')
        if isinstance(alt, (float,int,No)):
            self.__alterar_el = alt
        elif len(alt) == self.noI.ndim:
            self.__alterar_el = np.array(alt)
        else:
            raise ValueError(f'alt deve ser um ângulo (em rad), um No ou coordenadas de um ponto')
    ...
    def definir_carga(self,**kwargs):
        ...
        for carga, w in kwargs.items():
            if carga in self.__cargas_permitidas:
                ...
                raise ValueError(f'A carga {carga} não está entra as permitidas: {self.__cargas_permitidas}')
        return None
        ...
        for dTemp , T in kwargs.items():
            if dTemp in self.__var_temperatura:
                if dTemp == 'dT' and isinstance(T,(int,float)):
                    ...
                    raise ValueError(f'A variação de tempertaura {dTemp} tem formato incorreto')
            else:
                raise ValueError(f'A variação de temperatura {dTemp} não está entre as perimtidas: {self.__var_temperatura}')
    ...
    def carga_termica(self):
        ...
        cTemp= dict()
        if 'dT' in self.dTemp:
            if self.dTemp['dT'] != 0 : cTemp['dTm'] = self.dTemp['dT']

        if 'dT2' in self.dTemp:
            dT2 = self.dTemp['dT2']
            dT2_medio = (dT2[1] + dT2[0])/2
            dT2_var = (dT2[1] - dT2[0])/2
            if dT2_medio != 0 : cTemp['dTm'] = dT2_medio
            if dT2_var   != 0 : cTemp['dTv2'] = dT2_var
        ...
        if 'dT3' in self.dTemp:
            ...
            if dT2_var   != 0 : cTemp['dTv3'] = dT3_var
